...
This document describes how the Penguins ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`penguins acp` exposes an ACP agent over stdio and forwards prompts to a running
Penguins Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Penguins Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `penguins acp` over stdio.

Example config:

```bash
penguins config set gateway.remote.url wss://gateway-host:18789
penguins config set gateway.remote.token <token>
```

Example run:

```bash
penguins acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
penguins acp --session agent:main:main
penguins acp --session agent:design:main
penguins acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Penguins ACP” to start a thread.

## Execution Model

- ACP client spawns `penguins acp` and speaks ACP messages over stdio.
...
```bash
penguins acp --session agent:main:main
penguins acp --session-label "support inbox"
penguins acp --reset-session
```
...
`penguins acp` resolves the Gateway URL and auth from CLI flags or config:
