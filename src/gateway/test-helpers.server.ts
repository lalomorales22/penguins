...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.PENGUINS_STATE_DIR;
  previousConfigPath = process.env.PENGUINS_CONFIG_PATH;
  previousSkipBrowserControl = process.env.PENGUINS_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.PENGUINS_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.PENGUINS_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.PENGUINS_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.PENGUINS_SKIP_CHANNELS;
  previousSkipProviders = process.env.PENGUINS_SKIP_PROVIDERS;
  previousSkipCron = process.env.PENGUINS_SKIP_CRON;
  previousMinimalGateway = process.env.PENGUINS_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.PENGUINS_SKIP_PROVIDERS = "1";
  process.env.PENGUINS_SKIP_CRON = "1";
  process.env.PENGUINS_TEST_MINIMAL_GATEWAY = "1";
  process.env.PENGUINS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "penguins-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "penguins-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".penguins-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.PENGUINS_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.PENGUINS_CONFIG_PATH;
    } else {
      process.env.PENGUINS_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.PENGUINS_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.PENGUINS_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.PENGUINS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.PENGUINS_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.PENGUINS_SKIP_CANVAS_HOST;
    } else {
      process.env.PENGUINS_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.PENGUINS_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.PENGUINS_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.PENGUINS_SKIP_CHANNELS;
    } else {
      process.env.PENGUINS_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.PENGUINS_SKIP_PROVIDERS;
    } else {
      process.env.PENGUINS_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.PENGUINS_SKIP_CRON;
    } else {
      process.env.PENGUINS_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.PENGUINS_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.PENGUINS_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["PENGUINS_GATEWAY_TOKEN"]);
  const prev = process.env.PENGUINS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PENGUINS_GATEWAY_TOKEN = fallbackToken;
  }
