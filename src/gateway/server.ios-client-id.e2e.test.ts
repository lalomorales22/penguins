...
beforeAll(async () => {
  previousToken = process.env.PENGUINS_GATEWAY_TOKEN;
  process.env.PENGUINS_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.PENGUINS_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "penguins-ios", platform: "ios" },
  { clientId: "penguins-android", platform: "android" },
