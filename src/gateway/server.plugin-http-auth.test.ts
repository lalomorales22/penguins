...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.PENGUINS_CONFIG_PATH;
  const prevDisableCache = process.env.PENGUINS_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "penguins-plugin-http-auth-test-"));
  const configPath = path.join(dir, "penguins.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.PENGUINS_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.PENGUINS_DISABLE_CONFIG_CACHE;
    } else {
      process.env.PENGUINS_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
