...
import type { PenguinsConfig, ModelProviderConfig } from "../config/types.js";
import { resolvePenguinsAgentDir } from "../agents/agent-paths.js";
...
import { ensurePenguinsModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.PENGUINS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.PENGUINS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.PENGUINS_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<PenguinsConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.PENGUINS_AGENT_DIR,
  ...
  const hostAgentDir = resolvePenguinsAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-live-state-"));
  ...
  process.env.PENGUINS_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.penguins-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePenguinsAgentDir();
  const sanitizedCfg: PenguinsConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Penguins live tool probe (local, safe): " +
    ...
    process.env.PENGUINS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.PENGUINS_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensurePenguinsModelsJson(cfg);
      ...
      const rawModels = process.env.PENGUINS_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.penguins-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
