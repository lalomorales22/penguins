...
describe("gateway auth", () => {
  it("resolves token/password from PENGUINS gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          PENGUINS_GATEWAY_TOKEN: "env-token",
          PENGUINS_GATEWAY_PASSWORD: "env-password",
  ...
  it("does not resolve legacy CLAWDBOT gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          PENGUINS_GATEWAY_TOKEN: "legacy-token",
          PENGUINS_GATEWAY_PASSWORD: "legacy-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "none",
