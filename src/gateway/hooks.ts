...
export function resolveHooksConfig(cfg: PenguinsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: PenguinsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-penguins-token"] === "string"
      ? req.headers["x-penguins-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
