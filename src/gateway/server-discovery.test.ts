...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.PENGUINS_TAILNET_DNS;
    delete process.env.PENGUINS_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.PENGUINS_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.PENGUINS_TAILNET_DNS = "studio.tailnet.ts.net.";
