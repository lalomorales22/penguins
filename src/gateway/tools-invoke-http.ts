...
import { createPenguinsTools } from "../agents/penguins-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-penguins-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-penguins-account-id")?.trim() || undefined;
  ...
  const allTools = createPenguinsTools({
