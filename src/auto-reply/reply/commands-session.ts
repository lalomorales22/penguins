...
import { scheduleGatewaySigusr1Restart, triggerPenguinsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Penguins in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPenguinsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Penguins via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
