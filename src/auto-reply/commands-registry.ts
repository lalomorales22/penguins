...
export function isCommandEnabled(cfg: PenguinsConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PenguinsConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as PenguinsConfig),
...
export function getCommandDetection(_cfg?: PenguinsConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PenguinsConfig) {
