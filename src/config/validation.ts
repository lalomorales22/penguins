...
import type { PenguinsConfig, ConfigValidationIssue } from "./types.js";
...
import { PenguinsSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: PenguinsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: PenguinsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PenguinsSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PenguinsConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PenguinsConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as PenguinsConfig,
  };
}
