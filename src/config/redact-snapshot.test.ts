...
import type { ConfigFileSnapshot } from "./types.penguins.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.penguins/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/penguins/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/penguins/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/penguins/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/penguins/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = PenguinsSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "PenguinsConfig";
    const hints = mapSensitivePaths(PenguinsSchema, "", {});
