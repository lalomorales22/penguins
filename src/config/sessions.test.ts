...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-penguins",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-penguins#general");
  });
  ...
  it("derives session transcripts dir from PENGUINS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { PENGUINS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.PENGUINS_STATE_DIR = "/custom/state";
    try {
