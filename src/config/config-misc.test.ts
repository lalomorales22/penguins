...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = PenguinsSchema.safeParse({
      $schema: "https://penguins.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://penguins.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = PenguinsSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhook and cron.webhookToken", () => {
    const res = PenguinsSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      const configDir = path.join(home, ".penguins");
      await fs.mkdir(configDir, { recursive: true });
      await fs.writeFile(
        path.join(configDir, "penguins.json"),
        JSON.stringify({
          agents: { list: [{ id: "pi" }] },
