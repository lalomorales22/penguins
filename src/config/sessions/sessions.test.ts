...
describe("resolveStorePath", () => {
  ...
  it("uses OPENCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolved).toBe(
      path.resolve("/srv/penguins-home/.penguins/agents/research/sessions/sessions.json"),
    );
  });
});
...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/penguins/agents/main/sessions";
  ...
  it("accepts absolute sessionFile paths that resolve within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/penguins/agents/main/sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts absolute sessionFile with topic suffix within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/penguins/agents/main/sessions/abc-123-topic-42.jsonl" },
      { sessionsDir },
    );
  ...
  it("rejects absolute sessionFile paths outside known agent sessions dirs", () => {
    ...
    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/penguins/agents/work/not-sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
    ).toThrow(/within sessions directory/);
  });
...
describe("session store lock (Promise chain mutex)", () => {
  ...
  beforeAll(async () => {
    lockFixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "penguins-lock-test-"));
  });
