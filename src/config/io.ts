...
import type { PenguinsConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { comparePenguinsVersions } from "./version.js";
...
function coerceConfig(value: unknown): PenguinsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PenguinsConfig;
}
...
function stampConfigVersion(cfg: PenguinsConfig): PenguinsConfig {
...
function warnIfConfigFromFuture(cfg: PenguinsConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePenguinsVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Penguins (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as PenguinsConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PenguinsConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PenguinsConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PenguinsConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as PenguinsConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.PENGUINS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.PENGUINS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.PENGUINS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.PENGUINS_WATCH_SESSION === "string" &&
        deps.env.PENGUINS_WATCH_SESSION.trim().length > 0
          ? deps.env.PENGUINS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.PENGUINS_WATCH_COMMAND === "string" &&
        deps.env.PENGUINS_WATCH_COMMAND.trim().length > 0
          ? deps.env.PENGUINS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `PENGUINS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.PENGUINS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.PENGUINS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): PenguinsConfig {
