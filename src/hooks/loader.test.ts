...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.PENGUINS_BUNDLED_HOOKS_DIR;
    process.env.PENGUINS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.PENGUINS_BUNDLED_HOOKS_DIR;
    } else {
      process.env.PENGUINS_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
