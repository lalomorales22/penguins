...
`;
    ...
    expect(parsed.penguins.emoji).toBe("💾");
    expect(parsed.penguins.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.penguins.emoji).toBe("📝");
    expect(parsed.penguins.events).toEqual(["command"]);
    expect(parsed.penguins.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.penguins.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"penguins": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"penguins": {"events": ["test"]}}');
  });
...
describe("resolvePenguinsMetadata", () => {
  it("extracts penguins metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePenguinsMetadata(frontmatter);
  ...
  it("returns undefined when penguins key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        penguins: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Penguins" },
            { id: "npm", kind: "npm", package: "@penguins/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@penguins/hook");
  });
    ...
    "penguins":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Penguins" }],
      },
  }
---
...
`;
    ...
    const penguins = resolvePenguinsMetadata(frontmatter);
    expect(penguins).toBeDefined();
    expect(penguins?.emoji).toBe("💾");
    expect(penguins?.events).toEqual(["command:new"]);
    expect(penguins?.requires?.config).toEqual(["workspace.dir"]);
    expect(penguins?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const penguins = resolvePenguinsMetadata(frontmatter);
    expect(penguins?.emoji).toBe("disk");
    expect(penguins?.events).toEqual(["command:new"]);
  });
});
