...
This directory contains hooks that ship with Penguins. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.penguins/penguins.json`:
...
2. Restart gateway: `pkill -9 -f 'penguins.*gateway' && pnpm penguins gateway`
3. Enable the hook: `penguins hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.penguins.ai/automation/hooks
