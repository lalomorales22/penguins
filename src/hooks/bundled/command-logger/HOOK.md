...
homepage: https://docs.penguins.ai/automation/hooks#command-logger
metadata:
  {
    "penguins":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.penguins/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.penguins/logs/commands.log ~/.penguins/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/penguins`:
   ```
   /home/username/.penguins/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
