...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "penguins-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".penguins", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<PenguinsConfig> = {},
): PenguinsConfig {
  const base: PenguinsConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".penguins", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
