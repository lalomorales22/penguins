...
import { resolvePenguinsDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolvePenguinsAgentDir();
  await ensurePenguinsModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createPenguinsCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolvePenguinsDocsPath({
      workspaceDir: effectiveWorkspace,
