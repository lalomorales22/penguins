...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["PENGUINS_STATE_DIR"]);
  ...
  it("persists runs to disk and resumes after restart", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-subagent-"));
  ...
  it("uses isolated temp state when PENGUINS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "penguins-test-state"));
  });
});
