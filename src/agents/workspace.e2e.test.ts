...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses PENGUINS_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/penguins-home"), ".penguins", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".penguins", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("penguins-workspace-");
