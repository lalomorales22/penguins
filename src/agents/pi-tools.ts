...
function resolveExecConfig(params: { cfg?: PenguinsConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: PenguinsConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createPenguinsCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createPenguinsReadTool(freshReadTool);
