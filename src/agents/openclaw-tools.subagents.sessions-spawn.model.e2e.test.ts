...
} from "./penguins-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const callGatewayMock = getCallGatewayMock();

type CreatePenguinsTools = (typeof import("./penguins-tools.js"))["createPenguinsTools"];
type CreatePenguinsToolsOpts = Parameters<CreatePenguinsTools>[0];

async function getSessionsSpawnTool(opts: CreatePenguinsToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createPenguinsTools } = await import("./penguins-tools.js");
  const tool = createPenguinsTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("penguins-tools: subagents (sessions_spawn model + thinking)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
