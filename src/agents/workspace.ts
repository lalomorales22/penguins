...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.PENGUINS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".penguins", `workspace-${profile}`);
  }
  return path.join(home, ".penguins", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".penguins";
