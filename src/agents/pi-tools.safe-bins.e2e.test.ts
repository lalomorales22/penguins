...
describe("createPenguinsCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    if (process.platform === "win32") {
      return;
    }

    const { createPenguinsCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguins-safe-bins-"));
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    ...
    const { createPenguinsCodingTools } = await import("./pi-tools.js");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguins-safe-bins-expand-"));
