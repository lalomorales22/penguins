...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("penguins-ws-", async (workspaceDir) => {
      await withTempDir("penguins-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createPenguinsCodingTools({ workspaceDir });
    ...
    await withTempDir("penguins-ws-", async (workspaceDir) => {
      await withTempDir("penguins-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello penguins");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("penguins-ws-", async (workspaceDir) => {
      const tools = createPenguinsCodingTools({
        workspaceDir,
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("penguins-ws-", async (workspaceDir) => {
      await withTempDir("penguins-override-", async (overrideDir) => {
        const tools = createPenguinsCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("penguins-sandbox-", async (sandboxDir) => {
      await withTempDir("penguins-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "penguins-sbx-test",
        ...
        const tools = createPenguinsCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
