...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "penguins.sandbox=1");
  args.push("--label", `penguins.sessionKey=${params.scopeKey}`);
  args.push("--label", `penguins.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `penguins.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "penguins.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`penguins sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`penguins sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("penguins sandbox recreate --all");
}
