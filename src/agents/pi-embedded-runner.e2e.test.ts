...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies PenguinsConfig;

const ensureModels = (cfg: PenguinsConfig) => ensurePenguinsModelsJson(cfg, agentDir) as unknown;
