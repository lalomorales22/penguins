...
import { createPenguinsTools } from "./penguins-tools.js";
import { __testing, createPenguinsCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createPenguinsCodingTools();
...
describe("createPenguinsCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createPenguinsTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createPenguinsCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "penguins-outside.txt");
