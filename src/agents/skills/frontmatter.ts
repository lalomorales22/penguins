...
import {
  ...
  resolvePenguinsManifestInstall,
  resolvePenguinsManifestOs,
  resolvePenguinsManifestRequires,
...
export function resolvePenguinsMetadata(
  frontmatter: ParsedSkillFrontmatter,
): PenguinsSkillMetadata | undefined {
  const metadataObj = resolvePenguinsManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolvePenguinsManifestRequires(metadataObj);
  const install = resolvePenguinsManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolvePenguinsManifestOs(metadataObj);
  return {
