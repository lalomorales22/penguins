...
import { loadPenguinsPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `penguins-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.PENGUINS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "penguins.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.PENGUINS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadPenguinsPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.PENGUINS_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadPenguinsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
