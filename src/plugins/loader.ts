...
import { discoverPenguinsPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PenguinsPluginDefinition;
  register?: PenguinsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PenguinsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PenguinsPluginDefinition;
...
export function loadPenguinsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPenguinsPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "penguins/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "penguins/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PenguinsPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as PenguinsPluginModule;
    } catch (err) {
