import type { BrowserConfig, BrowserProfileConfig, PenguinsConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_PENGUINS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "penguins" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_PENGUINS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "penguins" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PENGUINS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PENGUINS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Penguins browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for penguins/penguins CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PenguinsConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PENGUINS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "penguins";
