...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/penguins" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "penguins",
        profiles: {
          penguins: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchPenguinsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolvePenguinsUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopPenguinsChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.PENGUINS_GATEWAY_PORT;
    process.env.PENGUINS_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.PENGUINS_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.PENGUINS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.PENGUINS_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.PENGUINS_GATEWAY_TOKEN;
    } else {
      process.env.PENGUINS_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.PENGUINS_GATEWAY_PASSWORD;
    } else {
      process.env.PENGUINS_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
