...
vi.mock("./chrome.js", () => ({
  resolvePenguinsUserDataDir: vi.fn(() => "/tmp/penguins-test/penguins/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolvePenguinsUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "penguins-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolvePenguinsUserDataDir).mockReturnValue(userDataDir);
