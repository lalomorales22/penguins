...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".penguins-profile-decorated");
}
 ...
 */
export function decoratePenguinsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PENGUINS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PENGUINS_BROWSER_COLOR).toUpperCase();
