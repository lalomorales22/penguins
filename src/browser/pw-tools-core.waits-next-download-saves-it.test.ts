...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredPenguinsTmpDir: vi.fn(() => "/tmp/penguins"),
}));
vi.mock("../infra/tmp-penguins-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredPenguinsTmpDir.mockReturnValue("/tmp/penguins");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredPenguinsTmpDir.mockReturnValue("/tmp/penguins-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedDownloadsTail = `${path.join("tmp", "penguins-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredPenguinsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "penguins-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "penguins-preferred", "downloads")}${path.sep}`),
    );
  });
