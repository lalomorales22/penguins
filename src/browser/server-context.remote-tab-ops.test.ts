...
vi.mock("./chrome.js", () => ({
  ...
  launchPenguinsChrome: vi.fn(async () => {
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("penguins");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const penguins = ctx.forProfile("penguins");

    const opened = await penguins.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("penguins")?.lastTargetId).toBe("CREATED");
  });
});
