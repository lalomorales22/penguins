...
describe("agent components", () => {
  const createCfg = (): PenguinsConfig => ({}) as PenguinsConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): PenguinsConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/penguins-sessions-test.json");
  });
