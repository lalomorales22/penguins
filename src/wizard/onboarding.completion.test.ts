...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "penguins",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/penguins.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("penguins");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "penguins");
