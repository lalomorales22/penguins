...
function listAgents(cfg: PenguinsConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: PenguinsConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: PenguinsConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<PenguinsConfig, EvaluatedBindingsCache>();
