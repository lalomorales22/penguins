...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.PENGUINS_STATE_DIR;
  const configPath = env.PENGUINS_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    PENGUINS_PROFILE: profile,
    PENGUINS_STATE_DIR: stateDir,
    PENGUINS_CONFIG_PATH: configPath,
    PENGUINS_GATEWAY_PORT: String(port),
    PENGUINS_GATEWAY_TOKEN: token,
    PENGUINS_LAUNCHD_LABEL: resolvedLaunchdLabel,
    PENGUINS_SYSTEMD_UNIT: systemdUnit,
    PENGUINS_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    PENGUINS_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    PENGUINS_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    PENGUINS_STATE_DIR: stateDir,
    PENGUINS_CONFIG_PATH: configPath,
    PENGUINS_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    PENGUINS_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    PENGUINS_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    PENGUINS_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    PENGUINS_LOG_PREFIX: "node",
    PENGUINS_SERVICE_MARKER: NODE_SERVICE_MARKER,
    PENGUINS_SERVICE_KIND: NODE_SERVICE_KIND,
    PENGUINS_SERVICE_VERSION: VERSION,
  };
}
