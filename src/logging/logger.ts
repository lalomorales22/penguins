...
import type { PenguinsConfig } from "../config/types.js";
import type { ConsoleStyle } from "./console.js";
import { resolvePreferredPenguinsTmpDir } from "../infra/tmp-penguins-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredPenguinsTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "penguins.log"); // legacy single-file path

const LOG_PREFIX = "penguins";
...
function resolveSettings(): ResolvedSettings {
  let cfg: PenguinsConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => PenguinsConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.PENGUINS_TEST_FILE_LOG !== "1" ? "silent" : "info";
