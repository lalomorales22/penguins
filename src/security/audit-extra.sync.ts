...
function summarizeGroupPolicy(cfg: PenguinsConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: PenguinsConfig): boolean {
...
function collectModels(cfg: PenguinsConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: PenguinsConfig): Set<string> {
  const baseCfg: PenguinsConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: PenguinsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: PenguinsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: PenguinsConfig): boolean {
...
function isBrowserEnabled(cfg: PenguinsConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: PenguinsConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep PENGUINS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("penguins security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer PENGUINS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const penguinsGatewayToken =
    typeof env.PENGUINS_GATEWAY_TOKEN === "string" && env.PENGUINS_GATEWAY_TOKEN.trim()
      ? env.PENGUINS_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-penguins-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: PenguinsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
