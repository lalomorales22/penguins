...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["PENGUINS_BASE_ENV"]);
    process.env.PENGUINS_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.PENGUINS_BASE_ENV ?? "") + "|" + (process.env.PENGUINS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { PENGUINS_TEST_ENV: "ok" },
        },
      );
