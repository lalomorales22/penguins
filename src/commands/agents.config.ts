...
export function listAgentEntries(cfg: PenguinsConfig): AgentEntry[] {
...
function resolveAgentName(cfg: PenguinsConfig, agentId: string) {
...
function resolveAgentModel(cfg: PenguinsConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PenguinsConfig): AgentSummary[] {
