...
import type { PenguinsConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PenguinsConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("penguins security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("penguins configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("penguins config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("penguins doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("penguins config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('penguins config set session.dmScope "per-channel-peer"') +
