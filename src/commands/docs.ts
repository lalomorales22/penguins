...
const SEARCH_TOOL = "https://docs.penguins.ai/mcp.SearchPenguins";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.penguins.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('penguins docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.penguins.ai/");
      runtime.log(`Search: ${formatCliCommand('penguins docs "your query"')}`);
    }
    return;
  }
