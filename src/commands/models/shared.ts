...
export async function updateConfig(
  mutator: (cfg: PenguinsConfig) => PenguinsConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: PenguinsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: PenguinsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("penguins agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
