...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvePenguinsAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("penguins models auth setup-token")}\` or \`${formatCliCommand("penguins configure")}\`.`
          : `Run \`${formatCliCommand("penguins configure")}\` or set an API key env var.`;
