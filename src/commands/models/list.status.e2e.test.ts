...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePenguinsAgentDir: vi.fn().mockReturnValue("/tmp/penguins-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/penguins-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/penguins-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePenguinsAgentDir: mocks.resolvePenguinsAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePenguinsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/penguins-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/penguins-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/penguins-agent-custom");
