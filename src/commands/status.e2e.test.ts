...
beforeAll(() => {
  envSnapshot = captureEnv(["PENGUINS_PROFILE"]);
...
vi.mock("../infra/penguins-root.js", () => ({
  resolvePenguinsPackageRoot: vi.fn().mockResolvedValue("/tmp/penguins"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/penguins/pnpm-lock.yaml",
      markerPath: "/tmp/penguins/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Penguins status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("penguins status --all") ||
          l.includes("penguins --profile isolated status --all") ||
          l.includes("penguins status --all") ||
          l.includes("penguins --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.PENGUINS_GATEWAY_TOKEN = "abcd1234";
    try {
