...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.PENGUINS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.PENGUINS_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.penguins.ai/gateway/health",
        "https://docs.penguins.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
...
async function promptWebToolsConfig(
  nextConfig: PenguinsConfig,
  runtime: RuntimeEnv,
): Promise<PenguinsConfig> {
  ...
  note(
    [
      ...
      "Docs: https://docs.penguins.ai/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Penguins update wizard" : "Penguins configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: PenguinsConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.penguins.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("penguins doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.PENGUINS_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.PENGUINS_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.PENGUINS_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.PENGUINS_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.PENGUINS_GATEWAY_TOKEN;
