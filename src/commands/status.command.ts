...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Penguins status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("penguins security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("penguins security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.penguins.ai/faq");
  runtime.log("Troubleshooting: https://docs.penguins.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("penguins status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("penguins logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("penguins status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("penguins gateway probe")}`);
  }
}
