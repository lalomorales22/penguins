...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.PENGUINS_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.PENGUINS_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "penguins");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.PENGUINS_STATE_DIR = stateDir;
    process.env.PENGUINS_CONFIG_PATH = path.join(stateDir, "penguins.json");
