...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.PENGUINS_OAUTH_DIR;
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      delete process.env.PENGUINS_OAUTH_DIR;
    } else {
      process.env.PENGUINS_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguins-wa-auth-"));
    try {
