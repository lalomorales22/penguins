...
async function withOnboardEnv(
  ...
  const prev = captureEnv([
    ...
    "PENGUINS_SKIP_CHANNELS",
    "PENGUINS_SKIP_GMAIL_WATCHER",
    "PENGUINS_SKIP_CRON",
    "PENGUINS_SKIP_CANVAS_HOST",
    "PENGUINS_GATEWAY_TOKEN",
    "PENGUINS_GATEWAY_PASSWORD",
    "CUSTOM_API_KEY",
    "PENGUINS_DISABLE_CONFIG_CACHE",
  ]);
  ...
  process.env.PENGUINS_DISABLE_CONFIG_CACHE = "1";
  delete process.env.PENGUINS_GATEWAY_TOKEN;
  delete process.env.PENGUINS_GATEWAY_PASSWORD;
  delete process.env.CUSTOM_API_KEY;

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), prefix));
  const configPath = path.join(tempHome, "penguins.json");
  process.env.HOME = tempHome;
  process.env.PENGUINS_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("penguins-onboard-minimax-", async ({ configPath, runtime }) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("penguins-onboard-minimax-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("penguins-onboard-zai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("penguins-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("penguins-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("penguins-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("penguins-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("penguins-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("penguins-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("penguins-onboard-litellm-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it.each([
    {
      ...
      prefix: "penguins-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "penguins-onboard-cf-gateway-infer-",
      options: {},
    },
  ])(
    "$name",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("penguins-onboard-together-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("penguins-onboard-qianfan-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("penguins-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "penguins-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "penguins-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "penguins-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "penguins-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("penguins-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "penguins-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
