...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PENGUINS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PENGUINS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PENGUINS_GATEWAY_TOKEN");
    expect(message).not.toContain("PENGUINS_GATEWAY_PASSWORD");
  });
