...
function resolveMode(cfg: PenguinsConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Penguins doctor");

  const root = await resolvePenguinsPackageRoot({
  ...
  let cfg: PenguinsConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("penguins configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("penguins config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("penguins setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("penguins doctor --fix")}" to apply changes.`);
  }
