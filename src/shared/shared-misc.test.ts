...
describe("shared/frontmatter", () => {
  ...
  test("resolvePenguinsManifestBlock parses JSON5 metadata and picks penguins block", () => {
    const frontmatter = {
      metadata: "{ penguins: { foo: 1, bar: 'baz' } }",
    };
    expect(resolvePenguinsManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolvePenguinsManifestBlock returns undefined for invalid input", () => {
    expect(resolvePenguinsManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolvePenguinsManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolvePenguinsManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
