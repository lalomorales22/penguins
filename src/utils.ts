...
export function resolveConfigDir(
  ...
  const override = env.PENGUINS_STATE_DIR?.trim() || env.PENGUINS_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".penguins");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.PENGUINS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$PENGUINS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via PENGUINS_STATE_DIR.
