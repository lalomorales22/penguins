...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "penguins-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.PENGUINS_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "penguins-telegram-"));
  process.env.PENGUINS_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.PENGUINS_STATE_DIR = previous;
    }
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as PenguinsConfig;
    const res = resolveTelegramToken(cfg);
