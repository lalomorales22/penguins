...
describe("update-startup", () => {
  ...
  let resolvePenguinsPackageRoot: (typeof import("./penguins-root.js"))["resolvePenguinsPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "PENGUINS_STATE_DIR");
    prevStateDir = process.env.PENGUINS_STATE_DIR;
    process.env.PENGUINS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolvePenguinsPackageRoot } = await import("./penguins-root.js"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    vi.mocked(resolvePenguinsPackageRoot).mockResolvedValue("/opt/penguins");
