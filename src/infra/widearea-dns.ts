...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.PENGUINS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*penguins-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "penguins";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "penguins");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "penguins-gw");
  ...
  records.push(`_penguins-gw._tcp IN PTR ${instanceLabel}._penguins-gw._tcp`);
  records.push(`${instanceLabel}._penguins-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._penguins-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; penguins-content-hash: ${contentHash}\n${contentBody}`;
}
