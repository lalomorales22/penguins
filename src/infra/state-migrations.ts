...
export async function autoMigrateLegacyStateDir(params: {
  ...
  if (env.PENGUINS_STATE_DIR?.trim() || env.PENGUINS_STATE_DIR?.trim()) {
    return { migrated: false, skipped: true, changes: [], warnings: [] };
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set PENGUINS_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.PENGUINS_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
