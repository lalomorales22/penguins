...
let ensurePenguinsCliOnPath: typeof import("./path-env.js").ensurePenguinsCliOnPath;

describe("ensurePenguinsCliOnPath", () => {
  const envKeys = [
    "PATH",
    "PENGUINS_PATH_BOOTSTRAPPED",
    "PENGUINS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensurePenguinsCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling penguins exists", () => {
    const tmp = abs("/tmp/penguins-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "penguins");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.PENGUINS_PATH_BOOTSTRAPPED;

    ensurePenguinsCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.PENGUINS_PATH_BOOTSTRAPPED = "1";
    ensurePenguinsCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/penguins-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "penguins");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/penguins-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "penguins");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "penguins");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/penguins-path/case-linuxbrew");
