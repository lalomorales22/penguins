...
describe("delivery-queue", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "penguins-dq-test-"));
  });
...
describe("resolveOutboundSessionRoute", () => {
  const baseConfig = {} as PenguinsConfig;
  ...
  it("treats Telegram usernames as DMs when unresolved", async () => {
    const cfg = { session: { dmScope: "per-channel-peer" } } as PenguinsConfig;
    const route = await resolveOutboundSessionRoute({
      cfg,
      channel: "telegram",
      agentId: "main",
