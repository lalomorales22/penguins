...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: PenguinsConfig | null = null;
  ...
  get(key: string, cfg: PenguinsConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: PenguinsConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: PenguinsConfig): void {
