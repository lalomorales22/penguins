...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__penguins_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "penguins-root");
...
describe("resolvePenguinsPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolvePenguinsPackageRootSync } = await import("./penguins-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "penguins");
    const pkgRoot = path.join(project, "node_modules", "penguins");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "penguins" }));

    expect(resolvePenguinsPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "penguins");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "penguins.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "penguins" }));

    expect(resolvePenguinsPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolvePenguinsPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-penguins package roots", async () => {
    const { resolvePenguinsPackageRootSync } = await import("./penguins-root.js");

    const pkgRoot = fx("not-penguins");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-penguins" }));

    expect(resolvePenguinsPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolvePenguinsPackageRoot } = await import("./penguins-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "penguins" }));

    await expect(resolvePenguinsPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
