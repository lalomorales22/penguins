...
function candidateBinDirs(opts: EnsurePenguinsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `penguins` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "penguins");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.PENGUINS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "penguins"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `penguins` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePenguinsCliOnPath(opts: EnsurePenguinsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.PENGUINS_PATH_BOOTSTRAPPED)) {
    return;
  }
