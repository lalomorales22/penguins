...
describe("resolveEffectiveHomeDir", () => {
  it("prefers PENGUINS_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/penguins-home"),
    );
  });
  ...
  it("expands PENGUINS_HOME when set to ~", () => {
    const env = {
      PENGUINS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for PENGUINS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { PENGUINS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when PENGUINS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ PENGUINS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { PENGUINS_HOME: "/srv/penguins-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/penguins-home")}/x`);
  });
