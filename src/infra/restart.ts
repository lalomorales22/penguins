...
export function triggerPenguinsRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PENGUINS_SYSTEMD_UNIT,
        process.env.PENGUINS_PROFILE,
      );
  ...
  const label =
    process.env.PENGUINS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PENGUINS_PROFILE);
