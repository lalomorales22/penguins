...
import { resolvePenguinsPackageRoot, resolvePenguinsPackageRootSync } from "./penguins-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvePenguinsPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "penguins" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath)) {
      try {
        ...
        if (parsed.name === "penguins" || parsed.name === "penguins") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolvePenguinsPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // penguins.mjs or dist/<bundle>.js
