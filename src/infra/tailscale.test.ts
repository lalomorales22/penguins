...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.PENGUINS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.PENGUINS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.PENGUINS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.PENGUINS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
