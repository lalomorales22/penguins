...
export function resolvePreferredPenguinsTmpDir(
  options: ResolvePreferredPenguinsTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "penguins" : `penguins-${uid}`;
    return path.join(base, suffix);
  };

  try {
    const preferred = lstatSync(POSIX_PENGUINS_TMP_DIR);
    if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
      return fallback();
    }
    accessSync(POSIX_PENGUINS_TMP_DIR, fs.constants.W_OK | fs.constants.X_OK);
    if (!isSecureDirForUser(preferred)) {
      return fallback();
    }
    return POSIX_PENGUINS_TMP_DIR;
  } catch (err) {
  ...
  try {
    ...
    mkdirSync(POSIX_PENGUINS_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_PENGUINS_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
