...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PenguinsConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PenguinsConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: PenguinsConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: PenguinsConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PenguinsConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PenguinsConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PenguinsConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
