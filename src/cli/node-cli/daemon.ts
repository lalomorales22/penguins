...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("penguins node install"), formatCliCommand("penguins node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("penguins node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    PENGUINS_LOG_PREFIX: baseEnv.PENGUINS_LOG_PREFIX ?? "node",
