...
vi.mock("../infra/penguins-root.js", () => ({
  resolvePenguinsPackageRoot: vi.fn(),
}));
...
const { resolvePenguinsPackageRoot } = await import("../infra/penguins-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "penguins-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("penguins-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolvePenguinsPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolvePenguinsPackageRoot).mockReset();
    ...
    vi.mocked(resolvePenguinsPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("penguins");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Penguins update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("penguins-update-wizard");
    const envSnapshot = captureEnv(["PENGUINS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.PENGUINS_GIT_DIR = tempDir;
