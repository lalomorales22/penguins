...
const SANDBOX_EXAMPLES = {
  main: [
    ["penguins sandbox list", "List all sandbox containers."],
    ["penguins sandbox list --browser", "List only browser containers."],
    ["penguins sandbox recreate --all", "Recreate all containers."],
    ["penguins sandbox recreate --session main", "Recreate a specific session."],
    ["penguins sandbox recreate --agent mybot", "Recreate agent containers."],
    ["penguins sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["penguins sandbox list", "List all sandbox containers."],
    ["penguins sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["penguins sandbox recreate --all", "Recreate all containers."],
    ["penguins sandbox recreate --session main", "Recreate a specific session."],
    ["penguins sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["penguins sandbox recreate --browser --all", "Recreate only browser containers."],
    ["penguins sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["penguins sandbox explain", "Show effective sandbox config."],
    ["penguins sandbox explain --session agent:main:main", "Explain a specific session."],
    ["penguins sandbox explain --agent work", "Explain an agent sandbox."],
    ["penguins sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.penguins.ai/cli/sandbox")}\n`,
    )
    .action(() => {
