...
const PENGUINS_REPO_URL = "https://github.com/penguins/penguins.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "penguins";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("penguins@")) {
    return trimmed.slice("penguins@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.PENGUINS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolvePenguinsPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", PENGUINS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `PENGUINS_GIT_DIR points at a non-git directory: ${params.dir}. Set PENGUINS_GIT_DIR to an empty folder or an penguins checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`PENGUINS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "penguins.mjs");
