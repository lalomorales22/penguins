import type { PenguinsApp } from "./app.ts";
...
export async function handleWhatsAppStart(host: PenguinsApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PenguinsApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PenguinsApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PenguinsApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PenguinsApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PenguinsApp): string {
...
function resolveGatewayHttpAuthHeader(host: PenguinsApp): string | null {
...
function buildGatewayHttpHeaders(host: PenguinsApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: PenguinsApp) {
...
export function handleNostrProfileToggleAdvanced(host: PenguinsApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: PenguinsApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PenguinsApp) {
  const state = host.nostrProfileFormState;
