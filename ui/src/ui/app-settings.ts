...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as PenguinsApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as PenguinsApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as PenguinsApp);
    await loadConfig(host as unknown as PenguinsApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as PenguinsApp, agentIds);
    }
    const agentId =
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as PenguinsApp);
    await loadDevices(host as unknown as PenguinsApp);
    await loadConfig(host as unknown as PenguinsApp);
    await loadExecApprovals(host as unknown as PenguinsApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as PenguinsApp);
    await loadConfig(host as unknown as PenguinsApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as PenguinsApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as PenguinsApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__PENGUINS_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    ...
    loadDebug(host as unknown as PenguinsApp),
  ]);
}
