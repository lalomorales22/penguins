...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/penguins/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/penguins");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/penguins/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PENGUINS_CONTROL_UI_BASE_PATH__ = "/penguins";
    const app = mountApp("/penguins/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/penguins");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/penguins/sessions");
  });
  ...
  it("updates the URL when clicking nav items", async () => {
    ...
    const link = app.querySelector<HTMLAnchorElement>('a.nav-item[href="/overview"]');
  ...
  it("maps legacy channels routes to overview", async () => {
