---
description: Update penguins from upstream when branch has diverged (ahead/behind)
---

# penguins Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "penguins" || true

# Move old version
mv /Applications/penguins.app /tmp/penguins-backup.app

# Install new build
cp -R dist/penguins.app /Applications/

# Launch
open /Applications/penguins.app
```
...
pnpm penguins health

# Verify no zombie processes
ps aux | grep -E "(penguins|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm penguins agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update penguins.json with fallback chains
...
pnpm penguins agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
