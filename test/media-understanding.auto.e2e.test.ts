...
import type { PenguinsConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("penguins-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("penguins-bin-sherpa-");
      const modelDir = await makeTempDir("penguins-sherpa-model-");
      ...
      const cfg: PenguinsConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("penguins-bin-whispercpp-");
      const modelDir = await makeTempDir("penguins-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("penguins-bin-gemini-");
      ...
      const cfg: PenguinsConfig = { tools: { media: { image: {} } } };
