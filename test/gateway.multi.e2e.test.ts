...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `penguins-e2e-${name}-`));
  const configDir = path.join(homeDir, ".penguins");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "penguins.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          PENGUINS_SKIP_CHANNELS: "1",
          PENGUINS_SKIP_BROWSER_CONTROL_SERVER: "1",
          PENGUINS_SKIP_CANVAS_HOST: "1",
        },
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-penguins-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-penguins-token": gwB.hookToken },
        ),
      ]);
