...
`penguins dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `penguins configure --section web`
...
If the config is invalid or contains legacy keys, the wizard asks you to run `penguins doctor` first.
</Note>
...
Use `penguins agents add <name>` to create a separate agent with its own workspace,
