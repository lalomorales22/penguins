...
Tlon is a decentralized messenger built on Urbit. Penguins connects to your Urbit ship and can
...
```bash
penguins plugins install @penguins/tlon
```

Local checkout (when running from a git repo):

```bash
penguins plugins install ./extensions/tlon
```
...
By default, Penguins blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `penguins message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Penguins replies in-thread.
