...
read_when:
  - Setting up Twitch chat integration for Penguins
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Penguins connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
penguins plugins install @penguins/twitch
```

Local checkout (when running from a git repo):

```bash
penguins plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "penguins", // Bot's Twitch account
