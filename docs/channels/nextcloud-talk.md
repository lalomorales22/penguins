...
```bash
penguins plugins install @penguins/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
penguins plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Penguins" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `penguins pairing list nextcloud-talk`
  - `penguins pairing approve nextcloud-talk <CODE>`
