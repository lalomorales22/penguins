...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Penguins to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
penguins plugins install @penguins/feishu
```

Local checkout (when running from a git repo):

```bash
penguins plugins install ./extensions/feishu
```
...
If you just installed Penguins, run the wizard:
...
2. Configuring app credentials in Penguins
...
1. You already ran `penguins channels add` for Feishu
2. The gateway is running (`penguins gateway status`)
...
```bash
penguins pairing approve feishu <CODE>
```
...
2. Run `penguins logs --follow` and look for `chat_id`
...
2. Run `penguins logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `penguins gateway status`  | Show gateway status           |
| `penguins gateway install` | Install/start gateway service |
| `penguins gateway stop`    | Stop gateway service          |
| `penguins gateway restart` | Restart gateway service       |
| `penguins logs --follow`   | Tail gateway logs             |
...
4. Check logs: `penguins logs --follow`
...
5. Ensure the gateway is running: `penguins gateway status`
6. Check logs: `penguins logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.penguins/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.penguins/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
