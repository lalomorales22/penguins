...
Matrix is an open, decentralized messaging protocol. Penguins connects as a Matrix **user**
...
```bash
penguins plugins install @penguins/matrix
```

Local checkout (when running from a git repo):

```bash
penguins plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `penguins plugins install @penguins/matrix`
   - From a local checkout: `penguins plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Penguins calls the same
     login endpoint, stores the access token in `~/.penguins/credentials/matrix/credentials.json`,
...
- On first connection, Penguins requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Penguins logs a warning.
...
`~/.penguins/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `penguins pairing list matrix`
  - `penguins pairing approve matrix <CODE>`
...
- On startup, Penguins resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
