...
read_when:
  - You want Penguins to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Penguins to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`penguins onboard`) and `penguins channels add` list optional channel plugins.
...
```bash
penguins plugins install @penguins/nostr
```

Use a local checkout (dev workflows):

```bash
penguins plugins install --link <path-to-penguins>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "penguins@example.com",
        "lud16": "penguins@example.com"
      }
    }
  }
}
