...
read_when:
  - You want Zalo Personal (unofficial) support in Penguins
...
Zalo Personal support for Penguins via a plugin, using `zca-cli` to automate a normal Zalo user account.
...
```bash
penguins plugins install @penguins/zalouser
```
...
```bash
penguins plugins install ./extensions/zalouser
