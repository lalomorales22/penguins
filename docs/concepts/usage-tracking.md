...
- `/usage cost` in chats: local cost summary aggregated from Penguins session logs.
- CLI: `penguins status --usage` prints a full per-provider breakdown.
- CLI: `penguins channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
