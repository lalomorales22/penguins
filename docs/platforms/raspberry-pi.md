---
summary: "Penguins on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Penguins on a Raspberry Pi
  - Running Penguins on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Penguins on Raspberry Pi

## Goal

Run a persistent, always-on Penguins Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Penguins features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled penguins

# Enable if not
sudo systemctl enable penguins

# Start on boot
sudo systemctl start penguins
```
...
```bash
# Check logs
journalctl -u penguins --no-pager -n 100

# Common fix: rebuild
cd ~/penguins  # if using hackable install
npm run build
sudo systemctl restart penguins
```
