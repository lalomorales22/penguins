...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Penguins
...
Running Penguins via Node with `tsx` fails at startup with:

```
[penguins] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `penguins status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch penguins.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node penguins.mjs status` works on Node 25.
