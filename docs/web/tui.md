...
```bash
penguins tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `penguins tui --deliver`
...
- Check the Gateway logs: `penguins logs --follow`.
- Confirm the agent can run: `penguins status` and `penguins models status`.
...
- No agents in picker: check `penguins agents list` and your routing config.
