...
Penguins integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Penguins owns lifecycle when `channels.signal.autoStart=true`.
...
- Penguins spawns `imsg rpc` as a child process (legacy iMessage integration).
