...
Penguins features that can generate provider usage or paid API calls.
...
- `penguins status --usage` and `penguins channels list` show provider **usage windows**
...
Penguins can pick up credentials from:
...
- `penguins status --usage`
- `penguins models status --json`
...
`penguins models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
