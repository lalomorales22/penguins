...
This document explains how Penguins manages sessions end-to-end:
...
Penguins is designed around a single **Gateway process** that owns session state.
...
Penguins persists sessions in two layers:
...
- Transcripts: `~/.penguins/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Penguins resolves these via `src/config/sessions.ts`.
...
Penguins intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Penguins consumes the events, but Pi decides when to compact).
...
Penguins also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Penguins bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Penguins leaves it alone.
...
- `penguins status` (CLI)
- `penguins sessions` / `sessions --json`
...
Penguins supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Penguins strips/suppresses this in the delivery layer.

As of `2026.1.10`, Penguins also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Penguins uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Penguins’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `penguins status`.
