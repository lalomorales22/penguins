...
(Or set `PENGUINS_PODMAN_QUADLET=1`; use `--container` to install only the container and launch script.)
...
If you ran `./setup-podman.sh --quadlet` (or `PENGUINS_PODMAN_QUADLET=1`), a [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html) unit is installed so the gateway runs as a systemd user service for the penguins user. The service is enabled and started at the end of setup.
...
- **Token:** Stored in `~penguins/.penguins/.env` as `PENGUINS_GATEWAY_TOKEN`. `setup-podman.sh` and `run-penguins-podman.sh` generate it if missing (uses `openssl`, `python3`, or `od`).
- **Optional:** In that `.env` you can set provider keys (e.g. `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other Penguins env vars.
- **Host ports:** By default the script maps `18789` (gateway) and `18790` (bridge). Override the **host** port mapping with `PENGUINS_PODMAN_GATEWAY_HOST_PORT` and `PENGUINS_PODMAN_BRIDGE_HOST_PORT` when launching.
- **Paths:** Host config and workspace default to `~penguins/.penguins` and `~penguins/.penguins/workspace`. Override the host paths used by the launch script with `PENGUINS_CONFIG_DIR` and `PENGUINS_WORKSPACE_DIR`.
...
- **Permission denied (EACCES) on config or auth-profiles:** The container defaults to `--userns=keep-id` and runs as the same uid/gid as the host user running the script. Ensure your host `PENGUINS_CONFIG_DIR` and `PENGUINS_WORKSPACE_DIR` are owned by that user.
