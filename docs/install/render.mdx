...
Deploy Penguins on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/penguins/penguins)
...
repository configures everything needed to run Penguins:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: PENGUINS_STATE_DIR
        value: /data/.penguins
      - key: PENGUINS_WORKSPACE_DIR
        value: /data/workspace
      - key: PENGUINS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/penguins`.
...
If you use the original Penguins repository, Render will not auto-deploy your Penguins. To update it, run a manual Blueprint sync from the dashboard.
...
For Penguins, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Penguins host.
