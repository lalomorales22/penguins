...
```bash
rm -rf "${PENGUINS_STATE_DIR:-$HOME/.penguins}"
```

If you set `PENGUINS_CONFIG_PATH` to a custom location outside the state dir, delete that file too.
...
- If you used profiles (`--profile` / `PENGUINS_PROFILE`), repeat step 3 for each state dir (defaults are `~/.penguins-<profile>`).
