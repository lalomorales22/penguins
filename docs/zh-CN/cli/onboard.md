...
summary: "`penguins onboard` 的 CLI 参考（交互式新手引导向导）"
title: onboard
x-i18n:
...
```bash
penguins onboard
penguins onboard --flow quickstart
penguins onboard --flow manual
penguins onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- 最快开始聊天：`penguins dashboard`（控制 UI，无需渠道设置）。
