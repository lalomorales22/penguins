...
你可以在 `~/.penguins/penguins.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/penguins.log"
  }
}
```
...
所有日志配置都在 `~/.penguins/penguins.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/penguins/penguins-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Penguins 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `penguins plugins enable diagnostics-otel` 启用插件。
...
- `penguins.tokens`（计数器，属性：`penguins.token`、`penguins.channel`、`penguins.provider`、`penguins.model`）
- `penguins.cost.usd`（计数器，属性：`penguins.channel`、`penguins.provider`、`penguins.model`）
- `penguins.run.duration_ms`（直方图，属性：`penguins.channel`、`penguins.provider`、`penguins.model`）
- `penguins.context.tokens`（直方图，属性：`penguins.context`、`penguins.channel`、`penguins.provider`、`penguins.model`）

消息流：

- `penguins.webhook.received`（计数器，属性：`penguins.channel`、`penguins.webhook`）
- `penguins.webhook.error`（计数器，属性：`penguins.channel`、`penguins.webhook`）
- `penguins.webhook.duration_ms`（直方图，属性：`penguins.channel`、`penguins.webhook`）
- `penguins.message.queued`（计数器，属性：`penguins.channel`、`penguins.source`）
- `penguins.message.processed`（计数器，属性：`penguins.channel`、`penguins.outcome`）
- `penguins.message.duration_ms`（直方图，属性：`penguins.channel`、`penguins.outcome`）

队列 + 会话：

- `penguins.queue.lane.enqueue`（计数器，属性：`penguins.lane`）
- `penguins.queue.lane.dequeue`（计数器，属性：`penguins.lane`）
- `penguins.queue.depth`（直方图，属性：`penguins.lane` 或 `penguins.channel=heartbeat`）
- `penguins.queue.wait_ms`（直方图，属性：`penguins.lane`）
- `penguins.session.state`（计数器，属性：`penguins.state`、`penguins.reason`）
- `penguins.session.stuck`（计数器，属性：`penguins.state`）
- `penguins.session.stuck_age_ms`（直方图，属性：`penguins.state`）
- `penguins.run.attempt`（计数器，属性：`penguins.attempt`）

### 导出的 span（名称 + 关键属性）

- `penguins.model.usage`
  - `penguins.channel`、`penguins.provider`、`penguins.model`
  - `penguins.sessionKey`、`penguins.sessionId`
  - `penguins.tokens.*`（input/output/cache_read/cache_write/total）
- `penguins.webhook.processed`
  - `penguins.channel`、`penguins.webhook`、`penguins.chatId`
- `penguins.webhook.error`
  - `penguins.channel`、`penguins.webhook`、`penguins.chatId`、`penguins.error`
- `penguins.message.processed`
  - `penguins.channel`、`penguins.outcome`、`penguins.chatId`、`penguins.messageId`、`penguins.sessionKey`、`penguins.sessionId`、`penguins.reason`
- `penguins.session.stuck`
  - `penguins.state`、`penguins.ageMs`、`penguins.queueDepth`、`penguins.sessionKey`、`penguins.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `penguins doctor`。
