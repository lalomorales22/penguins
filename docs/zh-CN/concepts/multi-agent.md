...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.penguins/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.penguins/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.penguins/penguins.json`（或 `OPENCLAW_CONFIG_PATH`）
- 状态目录：`~/.penguins`（或 `OPENCLAW_STATE_DIR`）
- 工作区：`~/.penguins/workspace`（或 `~/.penguins/workspace-<agentId>`）
- 智能体目录：`~/.penguins/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.penguins/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Penguins 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.penguins/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.penguins/workspace-<profile>`）。
- 状态默认为 `~/.penguins/agents/main/agent`。
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.penguins/workspace-alex" },
      { id: "mia", workspace: "~/.penguins/workspace-mia" },
    ],
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.penguins/workspace-home",
        agentDir: "~/.penguins/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.penguins/workspace-work",
        agentDir: "~/.penguins/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.penguins/credentials/whatsapp/personal
          // authDir: "~/.penguins/credentials/whatsapp/personal",
        },
        biz: {
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.penguins/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.penguins/workspace-opus",
