...
- `~/Library/Application Support/Penguins/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `penguins-canvas://<session>/<path>`

示例：

- `penguins-canvas://main/` → `<canvasRoot>/main/index.html`
- `penguins-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `penguins-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
penguins nodes canvas present --node <id>
penguins nodes canvas navigate --node <id> --url "/"
penguins nodes canvas eval --node <id> --js "document.title"
penguins nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__penguins__/a2ui/
```
...
penguins nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
penguins nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "penguins://agent?message=Review%20this%20design";
```
