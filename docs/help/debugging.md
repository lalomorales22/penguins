...
This is handy when you need to toggle obscure settings without editing `penguins.json`.
...
- **Global `--dev` (profile):** isolates state under `~/.penguins-dev` and
...
## Raw stream logging (Penguins)

Penguins can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.penguins/logs/raw-stream.jsonl
```
...
`~/.penguins/logs/raw-stream.jsonl`
