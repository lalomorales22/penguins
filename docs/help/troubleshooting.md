---
summary: "Symptom first troubleshooting hub for Penguins"
read_when:
  - Penguins is not working and you need the fastest path to a fix
...
- `penguins status` → shows configured channels and no obvious auth errors.
- `penguins status --all` → full report is present and shareable.
...
```mermaid
flowchart TD
  A[Penguins is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `penguins gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Browser tool fails">
    ...
    - `penguins` profile starts or `chrome` relay has an attached tab.
