...
read_when:
  - Designing workspace memory (~/.penguins/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Penguins integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.penguins/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.penguins/workspace` as canonical human-readable memory.
...
```
~/.penguins/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Penguins**, but keep a separable core library.

### Why integrate into Penguins?

- Penguins already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `penguins memory recall "…" --k 25 --since 30d`
  - `penguins memory reflect --since 7d`
...
Pragmatic take for `~/.penguins/workspace`:
