---
summary: "Chrome extension: let Penguins drive your existing Chrome tab"
read_when:
...
The Penguins Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate penguins-managed Chrome profile.
...
Penguins then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Penguins release (npm package) as static files. There is no separate “build” step.

After upgrading Penguins:

- Re-run `penguins browser extension install` to refresh the installed files under your Penguins state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Penguins ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Penguins to control.
...
- `ON`: attached; Penguins can drive that tab.
...
Debugging: `penguins sandbox explain`
...
`penguins browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `penguins browser extension install` first to copy the extension to a stable location under your Penguins state directory.
...
- **This is not isolated** like the dedicated penguins-managed profile.
