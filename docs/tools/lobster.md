---
title: Lobster
summary: "Typed workflow runtime for Penguins with resumable approval gates."
description: Typed workflow runtime for Penguins — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Penguins run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Penguins runs one Lobster tool call and gets a structured result.
...
Penguins launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | penguins.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
penguins.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Penguins tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Penguins Gateway (see the [Lobster repo](https://github.com/penguins/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Penguins keeps core tools enabled. To restrict core
...
→ penguins calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ penguins calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Penguins tools that do.
