---
summary: "Direct `penguins agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `penguins agent` (direct agent runs)

`penguins agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `penguins message --target`).
...
```bash
penguins agent --to +15555550123 --message "status update"
penguins agent --agent ops --message "Summarize logs"
...
penguins agent --to +15555550123 --message "Summon reply" --deliver
