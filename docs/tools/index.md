---
summary: "Agent tool surface for Penguins (browser, canvas, nodes, message, cron) replacing legacy `penguins-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `penguins-*` skills
title: "Tools"
---

# Tools (Penguins)

Penguins exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `penguins-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `penguins.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Penguins logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:penguins`: all built-in Penguins tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`penguins node run`).
...
- Requires a Brave API key (recommended: `penguins configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Penguins-managed browser.
...
- Quick smoke: `penguins nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `penguins gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Penguins clamps `tools.sessions.visibility` to `tree`.
