...
```bash
penguins config get agents.defaults.heartbeat.activeHours
penguins config get agents.defaults.heartbeat.activeHours.timezone
penguins config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
penguins cron list
penguins logs --follow
```
