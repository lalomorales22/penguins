...
- Jobs persist under `~/.penguins/cron/` so restarts don’t lose schedules.
...
penguins cron list
penguins cron run <job-id>
penguins cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.penguins/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `penguins cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Penguins defaults to `announce`.
...
- Job store: `~/.penguins/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.penguins/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
penguins cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
penguins cron edit <jobId> --agent ops
penguins cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
penguins cron run <jobId>
penguins cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
penguins cron edit <jobId> \
...
```bash
penguins system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`penguins system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `PENGUINS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Penguins applies exponential retry backoff for recurring jobs after consecutive errors:
