...
3. Penguins checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Penguins extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://penguins.example.com
    to: http://penguins-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
location / {
    ...
    proxy_pass http://penguins:18789;
...
`penguins security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Penguins config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `penguins security audit` and review findings
