...
description: "Complete field-by-field reference for ~/.penguins/penguins.json"
---

# Configuration Reference

Every field available in `~/.penguins/penguins.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Penguins uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `penguins doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["penguins-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `penguins plugins install @penguins/mattermost`.
...
Penguins spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@penguins"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `penguins.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Penguins auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Penguins assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.penguins/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.penguins/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@penguins"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:penguins`   | All built-in tools (excludes provider plugins)                                           |
...
Penguins uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.penguins/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `penguins onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `penguins onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `penguins onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `penguins onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `penguins onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `penguins onboard --auth-choice minimax-api`.
...
- Loaded from `~/.penguins/extensions`, `<workspace>/.penguins/extensions`, plus `plugins.load.paths`.
...
Convenience flags: `--dev` (uses `~/.penguins-dev` + port `19001`), `--profile <name>` (uses `~/.penguins-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.penguins/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-penguins-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__penguins__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__penguins__/a2ui/`
...
- Also serves A2UI at `/__penguins__/a2ui/`.
...
Writes a unicast DNS-SD zone under `~/.penguins/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `penguins dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.penguins/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.penguins/credentials/oauth.json`.
...
- Default log file: `/tmp/penguins/penguins-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `penguins doctor --fix` can strip unknown keys).
