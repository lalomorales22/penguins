...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `penguins configure`). If you are running Penguins via Podman using the dedicated `penguins` user, the config lives at `~penguins/.penguins/penguins.json`.
...
penguins approvals get --node <idOrNameOrIp>
penguins logs --follow
penguins status
```
...
penguins browser start --browser-profile penguins
...
```bash
penguins gateway status
penguins config get gateway.mode
penguins config get gateway.remote.url
penguins config get gateway.auth.mode
```
...
```bash
penguins config get gateway.bind
