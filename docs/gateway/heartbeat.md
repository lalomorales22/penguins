...
- During heartbeat runs, Penguins treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, Penguins skips the heartbeat run entirely (no model call).
