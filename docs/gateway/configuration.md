...
read_when:
  - Setting up Penguins for the first time
...
Penguins reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.penguins/penguins.json`.
...
<Tip>
**New to configuration?** Start with `penguins onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    penguins onboard       # full setup wizard
    penguins configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    penguins config get agents.defaults.workspace
    penguins config set agents.defaults.heartbeat.every "2h"
    penguins config unset tools.web.search.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.penguins/penguins.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Penguins only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`penguins doctor`, `penguins logs`, `penguins health`, `penguins status`)
- Run `penguins doctor` to see exact issues
- Run `penguins doctor --fix` (or `--yes`) to apply repairs
...
The Gateway watches `~/.penguins/penguins.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `penguins config set` for single keys.
    </Warning>
    ...
    penguins gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.penguins/workspace\" } } }",
      "baseHash": "<hash>",
...
Penguins reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.penguins/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Penguins runs your login shell and imports only the missing keys:
...
Env var equivalent: `PENGUINS_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${PENGUINS_GATEWAY_TOKEN}" } },
