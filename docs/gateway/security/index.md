...
## Quick check: `penguins security audit`
...
penguins security audit --deep
penguins security audit --fix
```
...
- Tighten local perms (`~/.penguins` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
If you run `--deep`, Penguins also attempts a best-effort live Gateway probe.
...
`penguins security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${PENGUINS_GATEWAY_PASSWORD}
```
...
Penguins stores session transcripts on disk under `~/.penguins/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.penguins` (see the audit section below). If you need
...
Penguins can refresh the skills list mid-session:
...
- If you install plugins from npm (`penguins plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.penguins/extensions/<pluginId>/` (or `$PENGUINS_STATE_DIR/extensions/<pluginId>/`).
  - Penguins uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
penguins pairing approve <channel> <code>
```
...
By default, Penguins routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Penguins has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.penguins/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.penguins or your logs.”
...
4. **Re-run audit**
   - `penguins security audit --deep` and confirm the report is clean.
...
- `~/.penguins/penguins.json`: `600` (user read/write only)
- `~/.penguins`: `700` (user only)

`penguins doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `PENGUINS_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__penguins__/canvas/` and `/__penguins__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_penguins-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `PENGUINS_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `penguins doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `PENGUINS_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `PENGUINS_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Penguins
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Penguins verifies the identity by resolving the
...
- Penguins will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.penguins/` (or `$PENGUINS_STATE_DIR/`) may contain secrets or private data:

- `penguins.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `penguins status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@penguins", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.penguins/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `penguins` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Penguins clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Penguins limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `penguins gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `PENGUINS_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/penguins/penguins-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.penguins/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Penguins version
...
Found a vulnerability in Penguins? Please report responsibly:

1. Email: [security@penguins.ai](mailto:security@penguins.ai)
