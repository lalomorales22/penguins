...
- If you install plugins from npm (`penguins plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.penguins/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
Assume anything under `~/.penguins/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:
