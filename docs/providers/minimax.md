---
summary: "Use MiniMax M2.1 in Penguins"
read_when:
  - You want MiniMax models in Penguins
...
```bash
penguins plugins enable minimax-portal-auth  # skip if already loaded.
penguins gateway restart  # restart if gateway is already running
penguins onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/penguins/penguins/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `penguins.json`:
...
- Use `penguins models list` and `penguins models set minimax/MiniMax-M2.1` to switch.
...
- Running `penguins configure` and selecting **MiniMax M2.1**, or
