---
summary: "Use Venice AI privacy-focused models in Penguins"
read_when:
  - You want privacy-focused inference in Penguins
...
```bash
penguins onboard --auth-choice venice-api-key
```
...
```bash
penguins chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Penguins shows all available Venice models. Pick based on your needs:
...
```bash
penguins models set venice/claude-opus-45
penguins models set venice/llama-3.3-70b
```

List all available models:

```bash
penguins models list | grep venice
```

## Configure via `penguins configure`

1. Run `penguins configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Penguins automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
penguins chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
penguins chat --model venice/claude-opus-45

# Use uncensored model
penguins chat --model venice/venice-uncensored

# Use vision model with image
penguins chat --model venice/qwen3-vl-235b-a22b

# Use coding model
penguins chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `penguins models list` to see currently available models. Some models may be temporarily offline.
