---
summary: "Penguins CLI reference for `penguins` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.penguins-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.penguins-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `penguins update` (source installs only).
...
Penguins uses a lobster palette for CLI output.
...
```
penguins [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `penguins voicecall`).

## Security

- `penguins security audit` — audit config + local state for common security foot-guns.
- `penguins security audit --deep` — best-effort live Gateway probe.
- `penguins security audit --fix` — tighten safe defaults and chmod state/config.

## Plugins

Manage extensions and their config:

- `penguins plugins list` — discover plugins (use `--json` for machine output).
- `penguins plugins info <id>` — show details for a plugin.
- `penguins plugins install <path|.tgz|npm-spec>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `penguins plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `penguins plugins doctor` — report plugin load errors.
...
- `penguins memory status` — show index stats.
- `penguins memory index` — reindex memory files.
- `penguins memory search "<query>"` — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.penguins/workspace`).
...
Non-interactive config helpers (get/set/unset). Running `penguins config` with no
...
- `penguins message send --target +15555550123 --message "Hi"`
- `penguins message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Penguins can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `penguins status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Penguins services are treated as first-class and aren't flagged as "extra".
...
penguins logs --limit 200
penguins logs --plain
penguins logs --json
penguins logs --no-color
```
...
`penguins models` is an alias for `models status`.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`penguins browser`](/cli/browser) and the [Browser tool](/tools/browser).
