...
- Publish as `penguins/plugin-sdk` (or export from core under `penguins/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PenguinsPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PenguinsConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PenguinsConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PenguinsConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PenguinsConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PenguinsConfig): string;
  };
};
```
...
- Introduce `penguins/plugin-sdk`.
- Add `api.runtime` to `PenguinsPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `penguinsRuntime: ">=2026.2.0"`).
