...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.penguins/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.penguins/node.json` (node id + pairing token).
...
Path: `~/.penguins/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.penguins/exec-approvals.sock",
...
- Unix socket at `~/.penguins/exec-approvals.sock` (0600).
