---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Penguins deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Penguins cron scheduling for periodic checks, or version status checks on a machine running Penguins (laptop, workstation, Pi, VPS).
---

# Penguins Host Hardening

## Overview

Assess and harden the host running Penguins, then align it to a user-defined risk tolerance without breaking access. Use Penguins security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Penguins changes the host firewall, SSH, or OS updates; it does not.
...
5. Penguins gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Penguins can access sensitive data.
...
### 2) Run Penguins security audits (read-only)

As part of the default read-only checks, run `penguins security audit --deep`. Only offer alternatives if the user requests them:

1. `penguins security audit` (faster, non-probing)
2. `penguins security audit --json` (structured output)

Offer to apply Penguins safe defaults (numbered):

1. `penguins security audit --fix`

Be explicit that `--fix` only tightens Penguins defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Penguins version/update status (read-only)

As part of the default read-only checks, run `penguins update status`.
...
- Credential hygiene notes (location of Penguins creds, prefer disk encryption)
...
- Penguins security audit (re-run)
...
After Penguins install or first hardening pass, run at least one baseline audit and version check:

- `penguins security audit`
- `penguins security audit --deep`
- `penguins update status`

Ongoing monitoring is recommended. Use the Penguins cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `penguins cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `penguins update status`
...
Before creating, `penguins cron list` and match on exact `name`. If found, `penguins cron edit <id> ...`.
If not found, `penguins cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `penguins update status` (preferred for source checkouts and channels)
2. `npm view penguins version` (published npm version)

## Penguins command accuracy

Use only supported commands and flags:

- `penguins security audit [--deep] [--fix] [--json]`
- `penguins status` / `penguins status --deep`
- `penguins health --json`
- `penguins update status`
- `penguins cron add|list|runs|run`

Do not invent CLI flags or imply Penguins enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Penguins compaction:
