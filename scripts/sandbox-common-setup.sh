...
BASE_IMAGE="${BASE_IMAGE:-penguins-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-penguins-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=penguins.sandbox=1)
NOTE
