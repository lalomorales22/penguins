...
#
# Suggested cron: */30 * * * * /home/admin/penguins/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Penguins notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., penguins-alerts)
...
STATE_FILE="$HOME/.penguins/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Penguins if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use penguins
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Penguins to $NOTIFY_PHONE..."
            penguins send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Penguins is down. Run: ssh l36 '~/penguins/scripts/mobile-reauth.sh'" "urgent"
    exit 1
