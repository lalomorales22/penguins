...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/penguins/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Penguins Re-Auth" -c "After getting key, run: ssh $SERVER '~/penguins/scripts/mobile-reauth.sh'" --id penguins-auth
        ;;
    *)
