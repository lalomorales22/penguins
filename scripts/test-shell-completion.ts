 ...
 * `penguins update`. Use it to verify the completion installation flow
...
\`penguins update\`, \`penguins doctor\`, and \`penguins onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Penguins Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`penguins completion --install\` later to enable.`));
    return;
  }
