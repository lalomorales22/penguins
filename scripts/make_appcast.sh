...
ZIP=${1:?"Usage: $0 Penguins-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/penguins/penguins/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Penguins-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/penguins/penguins/releases/download/v${VERSION}/"}
