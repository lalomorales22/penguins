...
OPENCLAW_AUTH="$HOME/.penguins/agents/main/agent/auth-profiles.json"
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        penguins_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$OPENCLAW_AUTH" 2>/dev/null || echo "0")
    fi
