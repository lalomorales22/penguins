...
# Penguins Auth Widget for Termux
...
SERVER="${PENGUINS_SERVER:-${PENGUINS_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Penguins auth..."

STATUS=$(ssh "$SERVER" '$HOME/penguins/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/penguins/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|PENGUINS_EXPIRING|PENGUINS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/penguins/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|PENGUINS_EXPIRED|PENGUINS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Penguins won't work until re-auth"
                ;;
        esac
        ;;
