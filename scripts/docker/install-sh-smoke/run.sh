...
INSTALL_URL="${PENGUINS_INSTALL_URL:-https://penguins.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${PENGUINS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PENGUINS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="penguins"
PACKAGE_NAME="${PENGUINS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PENGUINS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PENGUINS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${PENGUINS_INSTALL_LATEST_OUT:-}"
fi
