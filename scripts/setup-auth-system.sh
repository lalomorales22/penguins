#!/bin/bash
# Setup Penguins Auth Management System
...
echo "=== Penguins Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'penguins-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Penguins message: Send warning via Penguins itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/penguins-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/penguins-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/penguins-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now penguins-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status penguins-auth-monitor.timer"
echo ""
echo "Quick commands:"
