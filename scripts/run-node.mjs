...
const shouldBuild = (deps) => {
  if (deps.env.PENGUINS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.PENGUINS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[penguins] ${message}\n`);
};

const runPenguins = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["penguins.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
