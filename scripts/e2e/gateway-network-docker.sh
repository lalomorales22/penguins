...
IMAGE_NAME="penguins-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="penguins-net-e2e-$$"
GW_NAME="penguins-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "PENGUINS_GATEWAY_TOKEN=$TOKEN" \
	  -e "PENGUINS_SKIP_CHANNELS=1" \
	  -e "PENGUINS_SKIP_GMAIL_WATCHER=1" \
	  -e "PENGUINS_SKIP_CRON=1" \
	  -e "PENGUINS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
