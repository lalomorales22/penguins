...
This is an **alpha** iOS app that connects to an Penguins Gateway as a `role: node`.
...
3. On iOS: Penguins → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Penguins.xcodeproj -scheme Penguins -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/PenguinsKit` contains the shared transport/types used by the iOS app.
