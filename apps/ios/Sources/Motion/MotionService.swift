...
final class MotionService: MotionServicing {
    func activities(params: PenguinsMotionActivityParams) async throws -> PenguinsMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[PenguinsMotionActivityEntry], Error>) in
        ...
        return PenguinsMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: PenguinsPedometerParams) async throws -> PenguinsPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<PenguinsPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = PenguinsPedometerPayload(
