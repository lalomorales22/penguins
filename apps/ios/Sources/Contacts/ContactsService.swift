...
final class ContactsService: ContactsServicing {
    ...
    func search(params: PenguinsContactsSearchParams) async throws -> PenguinsContactsSearchPayload {
        ...
        return PenguinsContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: PenguinsContactsAddParams) async throws -> PenguinsContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return PenguinsContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return PenguinsContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> PenguinsContactPayload {
        PenguinsContactPayload(
