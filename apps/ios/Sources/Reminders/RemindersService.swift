...
final class RemindersService: RemindersServicing {
    func list(params: PenguinsRemindersListParams) async throws -> PenguinsRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[PenguinsReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return PenguinsReminderPayload(
        ...
        return PenguinsRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: PenguinsRemindersAddParams) async throws -> PenguinsRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = PenguinsReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return PenguinsRemindersAddPayload(reminder: payload)
    }
