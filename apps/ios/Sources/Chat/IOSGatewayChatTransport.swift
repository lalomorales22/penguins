...
struct IOSGatewayChatTransport: PenguinsChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PenguinsChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PenguinsChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PenguinsChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PenguinsChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PenguinsChatAttachmentPayload]) async throws -> PenguinsChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PenguinsChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(PenguinsChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(PenguinsGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinsGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinsChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PenguinsAgentEventPayload.self)
                        {
