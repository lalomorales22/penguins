...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> PenguinsNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> PenguinsNetworkStatusPayload {
        let status: PenguinsNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> PenguinsNetworkStatusPayload {
        PenguinsNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
