...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<PenguinsTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = PenguinsTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> PenguinsTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return PenguinsTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> PenguinsTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> PenguinsTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return PenguinsTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> PenguinsTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = PenguinsTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: PenguinsTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: PenguinsAgentEventPayload.self) else {
                continue
            }
