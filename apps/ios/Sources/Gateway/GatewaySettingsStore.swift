...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.penguins.gateway"
    private static let nodeService = "ai.penguins.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.penguins.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.penguins.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("penguins-gateway.log")
    }
