...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [PenguinsCapability.canvas.rawValue, PenguinsCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(PenguinsCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(PenguinsCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = PenguinsLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(PenguinsCapability.location.rawValue) }

        caps.append(PenguinsCapability.device.rawValue)
        caps.append(PenguinsCapability.photos.rawValue)
        caps.append(PenguinsCapability.contacts.rawValue)
        caps.append(PenguinsCapability.calendar.rawValue)
        caps.append(PenguinsCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(PenguinsCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            PenguinsScreenCommand.record.rawValue,
            PenguinsSystemCommand.notify.rawValue,
            PenguinsChatCommand.push.rawValue,
        ...
        if caps.contains(PenguinsCapability.camera.rawValue) {
            commands.append(PenguinsCameraCommand.list.rawValue)
            commands.append(PenguinsCameraCommand.snap.rawValue)
            commands.append(PenguinsCameraCommand.clip.rawValue)
        }
        if caps.contains(PenguinsCapability.location.rawValue) {
            commands.append(PenguinsLocationCommand.get.rawValue)
        }
        if caps.contains(PenguinsCapability.device.rawValue) {
            commands.append(PenguinsDeviceCommand.status.rawValue)
            commands.append(PenguinsDeviceCommand.info.rawValue)
        }
        if caps.contains(PenguinsCapability.photos.rawValue) {
            commands.append(PenguinsPhotosCommand.latest.rawValue)
        }
        if caps.contains(PenguinsCapability.contacts.rawValue) {
            commands.append(PenguinsContactsCommand.search.rawValue)
            commands.append(PenguinsContactsCommand.add.rawValue)
        }
        if caps.contains(PenguinsCapability.calendar.rawValue) {
            commands.append(PenguinsCalendarCommand.events.rawValue)
            commands.append(PenguinsCalendarCommand.add.rawValue)
        }
        if caps.contains(PenguinsCapability.reminders.rawValue) {
            commands.append(PenguinsRemindersCommand.list.rawValue)
            commands.append(PenguinsRemindersCommand.add.rawValue)
        }
        if caps.contains(PenguinsCapability.motion.rawValue) {
            commands.append(PenguinsMotionCommand.activity.rawValue)
            commands.append(PenguinsMotionCommand.pedometer.rawValue)
        }
