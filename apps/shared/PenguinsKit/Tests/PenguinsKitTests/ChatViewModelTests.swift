...
import Foundation
import Testing
@testable import PenguinsChatUI
...
private final class TestChatTransport: @unchecked Sendable, PenguinsChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PenguinsChatHistoryPayload]
    private let sessionsResponses: [PenguinsChatSessionsListResponse]

    private let stream: AsyncStream<PenguinsChatTransportEvent>
    private let continuation: AsyncStream<PenguinsChatTransportEvent>.Continuation

    init(
        historyResponses: [PenguinsChatHistoryPayload],
        sessionsResponses: [PenguinsChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<PenguinsChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PenguinsChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PenguinsChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [PenguinsChatAttachmentPayload]) async throws -> PenguinsChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return PenguinsChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PenguinsChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PenguinsChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PenguinsChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = PenguinsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = PenguinsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { PenguinsChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = PenguinsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = PenguinsChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = PenguinsChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { PenguinsChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
