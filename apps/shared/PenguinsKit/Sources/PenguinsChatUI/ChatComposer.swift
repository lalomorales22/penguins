...
@MainActor
struct PenguinsChatComposer: View {
    @Bindable var viewModel: PenguinsChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(PenguinsChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(PenguinsChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \PenguinsPendingAttachment.id)
                { (att: PenguinsPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            PenguinsPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(PenguinsChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PenguinsChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(PenguinsChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(PenguinsChatTheme.subtleCard)
        .clipShape(Capsule())
    }
