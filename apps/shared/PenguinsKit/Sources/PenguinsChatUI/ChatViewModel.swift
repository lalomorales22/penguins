...
private let chatUILogger = Logger(subsystem: "ai.penguins", category: "PenguinsChatUI")

@MainActor
@Observable
public final class PenguinsChatViewModel {
    public private(set) var messages: [PenguinsChatMessage] = []
    ...
    public var attachments: [PenguinsPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PenguinsChatPendingToolCall] = []
    public private(set) var sessions: [PenguinsChatSessionEntry] = []
    private let transport: any PenguinsChatTransport
    ...
    private var pendingToolCallsById: [String: PenguinsChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any PenguinsChatTransport) {
    ...
    public var sessionChoices: [PenguinsChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [PenguinsChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PenguinsPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PenguinsChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PenguinsChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [PenguinsChatMessage]) -> [PenguinsChatMessage] {
        var result: [PenguinsChatMessage] = []
    ...
    private static func dedupeKey(for message: PenguinsChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PenguinsChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> PenguinsChatAttachmentPayload in
            PenguinsChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PenguinsChatSessionEntry {
        PenguinsChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: PenguinsChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PenguinsChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PenguinsAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PenguinsChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PenguinsPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
