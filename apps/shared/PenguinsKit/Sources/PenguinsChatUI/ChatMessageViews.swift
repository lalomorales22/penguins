...
struct ChatMessageBubble: View {
    let message: PenguinsChatMessage
    let style: PenguinsChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? PenguinsChatTheme.userText : PenguinsChatTheme.assistantText
    ...
    private var inlineAttachments: [PenguinsChatMessageContent] {
    ...
    private var toolCalls: [PenguinsChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [PenguinsChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? PenguinsChatTheme.userBubble
        }
        if self.style == .onboarding {
            return PenguinsChatTheme.onboardingAssistantBubble
        }
        return PenguinsChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return PenguinsChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: PenguinsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? PenguinsChatTheme.userText : PenguinsChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: PenguinsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PenguinsChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(PenguinsChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [PenguinsChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: PenguinsChatTheme.assistantText)
            }
        }
    }
}
