...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/penguins")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("penguins-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("penguins-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/penguins/ (rolling or stdout), or nil if none exist.
