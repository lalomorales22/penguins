...
private let webChatSwiftLogger = Logger(subsystem: "ai.penguins", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: PenguinsChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PenguinsChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PenguinsChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PenguinsChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PenguinsGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    PenguinsAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PenguinsChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PenguinsChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PenguinsChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: PenguinsChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PenguinsChatView>) -> NSViewController
    {
