...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.penguins", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = PenguinsConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.penguinsExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "penguins CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "penguins@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "penguins@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "penguins@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "penguins@\(target)"])
            }

        statusHandler("Installing penguins@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed penguins@\(target)")
        } else {
