...
enum DebugActions {
    private static let verboseDefaultsKey = "penguins.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "penguins.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = PenguinsPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Penguins", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = PenguinsPaths.configURL
        guard
