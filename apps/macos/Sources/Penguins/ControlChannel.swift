...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PenguinsProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.penguins", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PenguinsKit.AnyCodable]()) {
                $0[$1.key] = PenguinsKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PenguinsProtocol.AnyCodable?) -> [String: PenguinsProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PenguinsProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PenguinsKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PenguinsProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("penguins.control.heartbeat")
    static let controlAgentEvent = Notification.Name("penguins.control.agent")
}
