...
enum CommandResolver {
    private static let projectRootDefaultsKey = "penguins.gatewayProjectRootPath"
    private static let helperName = "penguins"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let penguinsEntry = root.appendingPathComponent("penguins.mjs").path
        if FileManager().isReadableFile(atPath: penguinsEntry) { return penguinsEntry }
        let binEntry = root.appendingPathComponent("bin/penguins.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/penguins")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let penguinsPaths = self.penguinsManagedPaths(home: home)
        if !penguinsPaths.isEmpty {
            extras.insert(contentsOf: penguinsPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + penguinsPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func penguinsManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".penguins"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func penguinsExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPenguinsExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("penguins.mjs").path,
            root.appendingPathComponent("bin/penguins.js").path,
        ]
    ...
    static func hasAnyPenguinsInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.penguinsExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func penguinsNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let penguinsPath = self.projectPenguinsExecutable(projectRoot: root) {
                return [penguinsPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "penguins", subcommand] + extraArgs
            }
            if let penguinsPath = self.penguinsExecutable(searchPaths: searchPaths) {
                return [penguinsPath, subcommand] + extraArgs
            }

            let missingEntry = """
            penguins entrypoint missing (looked for dist/index.js or penguins.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real penguins CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/penguins"
        ...
        \(cliSection)
        \(projectSection)
        if command -v penguins >/dev/null 2>&1; then
          CLI="$(command -v penguins)"
          penguins \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/penguins.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/penguins.mjs"
            node "$PRJ/penguins.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/penguins.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/penguins.js"
            node "$PRJ/bin/penguins.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent penguins"
          pnpm --silent penguins \(quotedArgs);
        else
          echo "penguins CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? PenguinsConfigFile.loadDict()
