...
enum PenguinsLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = PenguinsOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PenguinsFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PenguinsLogging.bootstrapIfNeeded()
        let label = PenguinsLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PenguinsOSLogHandler: LogHandler {
...
struct PenguinsFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PenguinsLogging.parseLabel(self.label)
