...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.penguins", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: PenguinsPeekabooBridgeServices?
    private static var penguinsSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Penguins", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = PenguinsPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.penguinsSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.penguinsSocketPath, privacy: .public)")
    }
...
@MainActor
private final class PenguinsPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.penguins.peekaboo")
