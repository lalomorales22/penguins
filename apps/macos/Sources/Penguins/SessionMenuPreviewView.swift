...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.penguins", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PenguinsSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PenguinsSessionPreviewEntry,
    ...
    private static func cache(payload: PenguinsSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PenguinsChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PenguinsKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PenguinsKit.AnyCodable]) -> [PenguinsChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PenguinsChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PenguinsChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PenguinsChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PenguinsChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PenguinsChatMessage) -> String? {
