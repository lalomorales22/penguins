...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["penguins"]
        let text = "hey penguins"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey penguins do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["penguins"], minPostTriggerGap: 0.3)
