...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: PenguinsProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PenguinsProtocol.AnyCodable] = [
            "presence": .init([PenguinsProtocol.AnyCodable(entry)]),
        ]
        let payload = PenguinsProtocol.AnyCodable(payloadMap)
