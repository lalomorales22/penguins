...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: PenguinsProtocol.AnyCodable(["ok": PenguinsProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: PenguinsProtocol.AnyCodable(["ok": PenguinsProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = PenguinsProtocol.AnyCodable([
            "runId": PenguinsProtocol.AnyCodable("run-1"),
            "sessionKey": PenguinsProtocol.AnyCodable("main"),
            "state": PenguinsProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: PenguinsProtocol.AnyCodable(["a": PenguinsProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
