import type { PenguinsPluginApi, PenguinsPluginService } from "penguins/plugin-sdk";
...
function normalizeDenyList(cfg: PenguinsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: PenguinsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.allowCommands ?? [])]);
}

function patchConfigNodeLists(
  cfg: PenguinsPluginApi["config"],
  next: { allowCommands: string[]; denyCommands: string[] },
): PenguinsPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export default function register(api: PenguinsPluginApi) {
  ...
  const timerService: PenguinsPluginService = {
