...
metadata: { "penguins": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Penguins `sessions_spawn`
- **File I/O** == Penguins `read`/`write`
- **Remote fetch** == Penguins `web_fetch` (or `exec` with curl when POST is required)
