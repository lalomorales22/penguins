...
`penguins.invoke --each`) without adding new Penguins code per workflow.
...
This extension depends on Penguins internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Penguins extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.penguins/extensions` as a standalone plugin directory.
