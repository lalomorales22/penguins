...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as PenguinsConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as PenguinsConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as PenguinsConfig);
