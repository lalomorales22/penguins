/**
 * Twitch channel plugin for Penguins.
 *
...
import { buildChannelConfigSchema } from "penguins/plugin-sdk";
import type {
 ...
 * for Penguins. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PenguinsConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PenguinsConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PenguinsConfig): boolean => {
