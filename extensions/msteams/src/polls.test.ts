import type { PluginRuntime } from "penguins/plugin-sdk";
...
const runtimeStub = {
  state: {
    resolveStateDir: (env: NodeJS.ProcessEnv = process.env, homedir?: () => string) => {
      ...
      return path.join(resolvedHome, ".penguins");
    },
  },
...
describe("msteams polls", () => {
  ...
  it("stores and records poll votes", async () => {
    const home = await fs.promises.mkdtemp(path.join(os.tmpdir(), "penguins-msteams-polls-"));
