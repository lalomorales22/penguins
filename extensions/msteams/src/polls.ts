...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["penguinsPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["penguins", "pollId"]) ??
    readNestedString(value, ["penguins", "poll", "id"]) ??
    readNestedString(value, ["data", "penguinsPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "penguins", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          penguinsPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "penguins poll vote",
          displayText: "Vote recorded",
          value: { penguinsPollId: pollId, pollId },
        },
      },
    ],
  };
