...
Penguins extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
penguins channels login --channel zalouser              # Login via QR
penguins channels login --channel zalouser --account work
...
```bash
penguins directory self --channel zalouser
penguins directory peers list --channel zalouser --query "name"
penguins directory groups list --channel zalouser --query "work"
penguins directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
penguins message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
penguins message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `penguins channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
penguins channels login --channel zalouser --account work
penguins message send --channel zalouser --account work --target <id> --message "Hello"
